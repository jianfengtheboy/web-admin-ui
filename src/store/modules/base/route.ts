...
export const loadView = (view: string) => {
	...
	for (const path in modules) {
		const dir = path.split('views/main/')[1].split('.vue')[0]
		if (dir === view) {
			result = () => modules[path]()
		}
	}
 ...
 */
const formatAsyncRoutes = (menus: IMenuItem[]) => {
...
const storeSetup = () => {
	...
	// 合并路由
	const setRoutes = (routeArr: RouteRecordRaw[]) => {
		routes.value = constantRoutes.concat(routeArr)
		asyncRoutes.value = routeArr
	}
