...
const search = () => {
	if (searchValue.value) {
		...
		searchList.value = IconList.filter(item => {
			return item
				.toLowerCase()
				.startsWith(`${props.type === 'arco' ? (temp.startsWith('icon') ? '' : 'icon') + temp : temp}`)
		})
...
<template>
	<a-popover trigger="click">
		...
		>
			<template #prefix>
				<template v-if="modelValue">
					<component v-if="props.type === 'arco' && modelValue" :size="16" :is="modelValue" />
					<BaseIcon v-if="props.type === 'custom' && modelValue" :size="16" :name="modelValue" />
				</template>
				<icon-search v-else />
			</template>
		</a-input>
		...
		<template #content>
			<div class="container overflow-hidden" :class="{ 'is-list': !isGridView }">
				<a-row>
					...
					<a-row wrap :gutter="4">
						<a-col :span="isGridView ? 4 : 8" v-for="item of currentPageIconList" :key="item">
							<div class="icon-item" :class="{ active: modelValue === item }" @click="handleSelectedIcon(item)">
								...
								<BaseIcon v-if="props.type === 'custom'" :name="item" :size="20" />
								<div class="icon-name overflow-hidden whitespace-nowrap text-ellipsis">{{ item }}</div>
							</div>
						</a-col>
					</a-row>
				</section>
...
.container {
	...
	.icon-list {
		...
		.icon-item {
			...
			align-items: center;
			cursor: pointer;
			border: 1px dashed var(--color-bg-popup);
